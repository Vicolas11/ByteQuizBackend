...
const LogoutUserController = catchAsync(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const allTokens = await prisma.logoutToken.findMany();
    const tokenExist = await prisma.logoutToken.findUnique({
      where: { token },
    });

    if (!tokenExist) {
      await prisma.logoutToken.create({ data: { token, userId } });
    }
    ...
    allTokens.map(({ token }) => {
      verify(token, secretKey, async (err, _) => {
        if (err) {
          ...
          await prisma.logoutToken.delete({ where: { token } });
        }
      });
    });
